...
impl DiagnosticsState {
    ...
    fn latest_report(&self) -> Option<PathBuf> {
        self.store
            .lock()
            .expect("diagnostics lock")
            .latest_report
            .clone()
            .or_else(|| newest_report(&self.root))
    }
    ...
    pub fn start_run(&self, source: impl Into<String>, context: Value) -> Result<String> {
        ...
        fs::create_dir_all(&report_dir).context("failed to create diagnostics run directory")?;
        let json_path = report_dir.join(format!("{run_id}.json"));
        let markdown_path = report_dir.join(format!("{run_id}.md"));
        let mut run = DiagnosticRun {
            ...
            evidence: json!({
                ...
                "note": "Raw tracing logs are complementary evidence. The structured run report is the primary reconstruction artifact."
            }),
            privacy: json!({
                "secrets": "Password/API-key/token/authorization-like fields are replaced with <redacted>.",
                "paths": "The current user's home directory is replaced with $HOME when possible.",
                "audio": "Audio bytes are never copied into the diagnostic report.",
                "accuracy": "Without a human reference, quality findings are anomaly signals rather than measured accuracy."
            }),
            report_files: DiagnosticReportPaths {
        ...
        run.events.push(event("info", "lifecycle", "run.start", "Diagnostic run started", json!({})));
    ...
    pub fn record_event(
        ...
        if run.events.len() >= MAX_EVENTS_PER_RUN {
            if !run.anomalies.iter().any(|item| item.code == "event_limit_reached") {
                run.anomalies.push(anomaly(
                    "event_limit_reached",
                    "warning",
                    &format!("Diagnostic event limit ({MAX_EVENTS_PER_RUN}) reached."),
                    json!({}),
                ));
                persist_run(run, false)?;
            }
        ...
        run.events.push(event(severity, category, stage, message, sanitize_value(data)));
        persist_run(run, false)
    }
    ...
    pub fn add_anomaly(&self, run_id: &str, item: DiagnosticAnomaly) -> Result<()> {
        let mut store = self.store.lock().expect("diagnostics lock");
        let Some(run) = store.active.get_mut(run_id) else {
            return Ok(());
        };
        let item = DiagnosticAnomaly {
            evidence: sanitize_value(item.evidence),
            ..item
        };
        if !run
            .anomalies
            .iter()
            .any(|existing| existing.code == item.code && existing.evidence == item.evidence)
        {
            run.anomalies.push(item);
            persist_run(run, false)?;
        }
        Ok(())
    }
    ...
    pub fn finish_run(&self, run_id: &str, outcome: &str, result: Value) -> Result<Option<DiagnosticReportPaths>> {
        ...
        run.events.push(event(
            ...
            "lifecycle",
            "run.finish",
            &format!("Diagnostic run finished with outcome: {outcome}"),
            json!({}),
        ));
    ...
    pub fn record_crash_best_effort(&self, message: &str) {
        ...
        for run in store.active.values_mut() {
            ...
            run.finished_at = Some(Utc::now().to_rfc3339());
            run.duration_ms = Some(run.started_instant.elapsed().as_millis());
            run.events.push(event("error", "crash", "process.crash", message, json!({})));
        ...
        for id in ids {
            if let Some(mut run) = store.active.remove(&id) {
                ...
                run.events.push(event(
                    ...
                    "lifecycle",
                    "run.interrupted",
                    &format!("Run finalized because the application ended: {outcome}"),
                    json!({}),
                ));
...
pub fn get_latest_diagnostic_report(state: State<'_, DiagnosticsState>) -> Option<DiagnosticReportPaths> {
    let json_path = state.latest_report()?;
    Some(DiagnosticReportPaths {
        markdown: sanitize_path_string(&json_path.with_extension("md")),
        json: sanitize_path_string(&json_path),
    })
}

#[tauri::command]
pub fn get_latest_diagnostic_report_content(
    state: State<'_, DiagnosticsState>,
) -> std::result::Result<Option<String>, String> {
    let Some(path) = state.latest_report() else {
        return Ok(None);
    };
...
pub fn show_diagnostics_folder(state: State<'_, DiagnosticsState>) -> std::result::Result<(), String> {
    showfile::show_path_in_file_manager(&state.root);
    Ok(())
}
...
pub fn analyze_transcript(segments: &[Segment], media_duration_seconds: Option<f64>) -> (Value, Vec<DiagnosticAnomaly>) {
    ...
    for segment in segments {
        ...
        if previous_start.is_some_and(|start| segment.start < start) {
    ...
    let metrics = json!({
        ...
        "accuracy_note": "Structural anomaly signals only; no WER/CER is claimed without a reference transcript.",
    ...
    if duration_seconds >= 30.0 && largest_gap_cs >= 1_500 {
        anomalies.push(anomaly(
            "large_temporal_gap",
            "warning",
            "A gap of at least 15 seconds exists between transcript segments. It may be legitimate silence and requires audio inspection.",
    ...
    if duration_seconds >= 30.0 && coverage_ratio < 0.20 && !segments.is_empty() {
        anomalies.push(anomaly(
            ...
            "warning",
            "Transcript timestamps cover less than 20% of the media timeline. This is a signal, not proof of missing speech.",
    ...
    if adjacent_duplicate_ratio >= 0.35 && adjacent_exact_duplicates >= 3 {
        anomalies.push(anomaly(
            ...
            "warning",
            "Many consecutive transcript segments contain identical text. This can be decoder degeneration or legitimate repeated speech.",
            json!({ "count": adjacent_exact_duplicates, "ratio": adjacent_duplicate_ratio }),
        ));
    }
    if let Some((text, count)) = dominant {
        if count >= 5 && count * 2 >= segments.len().max(1) {
            anomalies.push(anomaly(
                ...
                "warning",
                "One text segment dominates a large share of the output. This is a repetition signal, not automatic hallucination.",
...
fn sanitize_value_with_key(value: Value, key: Option<&str>) -> Value {
    ...
    match value {
        ...
        Value::Array(values) => Value::Array(
            values
                .into_iter()
                .map(|value| sanitize_value_with_key(value, key))
                .collect(),
        ),
        Value::String(value) => {
            let value = redact_home(&value);
            if key.is_some_and(|key| key.to_ascii_lowercase().contains("prompt")) && value.chars().count() > 500 {
...
fn persist_run(run: &DiagnosticRun, include_markdown: bool) -> Result<()> {
    ...
    fs::write(
        &tmp_path,
        serde_json::to_vec_pretty(run).context("failed to serialize diagnostic report")?,
    )
    .context("failed to write diagnostic report snapshot")?;
    if json_path.exists() {
    ...
    if include_markdown {
        fs::write(run.report_dir.join(format!("{}.md", run.run_id)), render_markdown(run))
            .context("failed to write diagnostic markdown report")?;
    }
    Ok(())
}
...
fn render_markdown(run: &DiagnosticRun) -> String {
    ...
    for item in &run.events {
        output.push_str(&format!(
            ...
            item.severity,
            item.stage,
            markdown_cell(&item.message)
        ));
    }
    output.push_str("\n## Result\n\n```json\n");
    output.push_str(&serde_json::to_string_pretty(&run.result).unwrap_or_default());
    output.push_str("\n```\n\n## Evidence & privacy\n\n```json\n");
    let evidence = json!({ "evidence": &run.evidence, "privacy": &run.privacy });
    output.push_str(&serde_json::to_string_pretty(&evidence).unwrap_or_default());
    output.push_str("\n```\n");
    output
}

fn event(severity: &str, category: &str, stage: &str, message: &str, data: Value) -> DiagnosticEvent {
    DiagnosticEvent {
...
mod tests {
    ...
    fn sanitizer_redacts_secrets_and_home_paths() {
        ...
        let path = home
            .clone()
            .map(|home| format!("{home}/audio.wav"))
            .unwrap_or_else(|| "/tmp/audio.wav".to_string());
    ...
    #[test]
    fn transcript_analysis_detects_repetition_signals_without_claiming_inaccuracy() {
        let segments = vec![
